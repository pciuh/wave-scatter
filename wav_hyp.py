...
def plojoint(fndat,fpdf,mu,p_title,xLim,yLim,dxy,Mar='o',p_size=20):
    ...
    ax.scatter(fndat[:,0],fndat[:,1],marker=Mar,alpha=p_alpha,
...
o_title = 'Scatter Data (Atlantinc Marine Energy Test Site)'
...
pht = df.replace(0,np.nan).values
vteg,vhsg = np.meshgrid(vte,vhs)

fig = plojoint(np.array([vteg.flatten(),vhsg.flatten()]).T,pht.flatten(),mu,o_title,xLim,xLim,dxy,'s',70)
fig.savefig('scat-a.png',dpi=300)

fig = plojoint(fndat,fpdf,mu,p_title,xLim,xLim,dxy)
